...
$(function () {
  ...
  var getValueChange = function (slider, value_change, type = true) {
    let value =
      slider >= value_change ? slider - value_change : value_change - slider;
  ...
  $(document).on("keyup", ".popup-quota-update .quota-inputtext", function () {
    ...
    $(document)
      ...
      .val(getValueChange($(this).val(), $(this).attr("data-default"), false));
  });
  $(document).on(
    "change input",
    ...
    function () {
      ...
      $(".company-quota-box p").html(
        dataType(
          $(".popup-quota-update .quota-box").attr("data-max") - $(this).val(),
          true
        )
      );
  ...
  $(document).on("focus", ".text-field-dynamic", function () {
    $(this).parent().find("label").addClass("v-label--active");
    // console.log("v-label--active ");
  });

  $(document).on("focusout", ".text-field-dynamic", function () {
    if ($(this).val().length == 0)
      $(this).parent().find("label").removeClass("v-label--active");
    // console.log("v-label--active ");
  });
});
